from typing import Any, Dict, Optional, Union, cast

import httpx

from ...client import Client
from ...models.api_response_v1 import ApiResponseV1
from ...types import UNSET, Response, Unset


def _get_kwargs(
    forum_id: int,
    *,
    client: Client,
    json_body: Any,
    with_first_post: Union[Unset, None, bool] = UNSET,
) -> Dict[str, Any]:
    url = "{}/forums/{forum_id}/list".format(client.base_url, forum_id=forum_id)

    headers: Dict[str, str] = client.get_headers()
    cookies: Dict[str, Any] = client.get_cookies()

    params: Dict[str, Any] = {}
    params["with_first_post"] = with_first_post

    params = {k: v for k, v in params.items() if v is not UNSET and v is not None}

    json_json_body = json_body

    return {
        "method": "post",
        "url": url,
        "headers": headers,
        "cookies": cookies,
        "timeout": client.get_timeout(),
        "json": json_json_body,
        "params": params,
    }


def _parse_response(*, response: httpx.Response) -> Optional[Union[Any, ApiResponseV1]]:
    if response.status_code == 200:
        response_200 = cast(Any, response.json())
        return response_200
    if response.status_code == 400:
        response_400 = ApiResponseV1.from_dict(response.json())

        return response_400
    if response.status_code == 404:
        response_404 = cast(Any, None)
        return response_404
    return None


def _build_response(*, response: httpx.Response) -> Response[Union[Any, ApiResponseV1]]:
    return Response(
        status_code=response.status_code,
        content=response.content,
        headers=response.headers,
        parsed=_parse_response(response=response),
    )


def sync_detailed(
    forum_id: int,
    *,
    client: Client,
    json_body: Any,
    with_first_post: Union[Unset, None, bool] = UNSET,
) -> Response[Union[Any, ApiResponseV1]]:
    """list topics

    Args:
        forum_id (int):
        with_first_post (Union[Unset, None, bool]):
        json_body (Any):

    Returns:
        Response[Union[Any, ApiResponseV1]]
    """

    kwargs = _get_kwargs(
        forum_id=forum_id,
        client=client,
        json_body=json_body,
        with_first_post=with_first_post,
    )

    response = httpx.request(
        verify=client.verify_ssl,
        **kwargs,
    )

    return _build_response(response=response)


def sync(
    forum_id: int,
    *,
    client: Client,
    json_body: Any,
    with_first_post: Union[Unset, None, bool] = UNSET,
) -> Optional[Union[Any, ApiResponseV1]]:
    """list topics

    Args:
        forum_id (int):
        with_first_post (Union[Unset, None, bool]):
        json_body (Any):

    Returns:
        Response[Union[Any, ApiResponseV1]]
    """

    return sync_detailed(
        forum_id=forum_id,
        client=client,
        json_body=json_body,
        with_first_post=with_first_post,
    ).parsed


async def asyncio_detailed(
    forum_id: int,
    *,
    client: Client,
    json_body: Any,
    with_first_post: Union[Unset, None, bool] = UNSET,
) -> Response[Union[Any, ApiResponseV1]]:
    """list topics

    Args:
        forum_id (int):
        with_first_post (Union[Unset, None, bool]):
        json_body (Any):

    Returns:
        Response[Union[Any, ApiResponseV1]]
    """

    kwargs = _get_kwargs(
        forum_id=forum_id,
        client=client,
        json_body=json_body,
        with_first_post=with_first_post,
    )

    async with httpx.AsyncClient(verify=client.verify_ssl) as _client:
        response = await _client.request(**kwargs)

    return _build_response(response=response)


async def asyncio(
    forum_id: int,
    *,
    client: Client,
    json_body: Any,
    with_first_post: Union[Unset, None, bool] = UNSET,
) -> Optional[Union[Any, ApiResponseV1]]:
    """list topics

    Args:
        forum_id (int):
        with_first_post (Union[Unset, None, bool]):
        json_body (Any):

    Returns:
        Response[Union[Any, ApiResponseV1]]
    """

    return (
        await asyncio_detailed(
            forum_id=forum_id,
            client=client,
            json_body=json_body,
            with_first_post=with_first_post,
        )
    ).parsed
